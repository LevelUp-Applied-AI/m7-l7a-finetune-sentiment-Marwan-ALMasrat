...
def evaluate_classifier(trainer: Trainer, tokenized_test) -> dict:
    ...
    per_f1   = f1_score(labels, pred_idx, average=None)
    per_prec = precision_score(labels, pred_idx, average=None, zero_division=0)
    per_rec  = recall_score(labels, pred_idx, average=None, zero_division=0)

    return {
        "accuracy": float(accuracy),
        "macro_f1": float(macro_f1),
        "per_class_f1":        {id2label[i]: float(v) for i, v in enumerate(per_f1)},
        "per_class_precision":  {id2label[i]: float(v) for i, v in enumerate(per_prec)},
        "per_class_recall":     {id2label[i]: float(v) for i, v in enumerate(per_rec)},
    }
...
def main() -> None:
    ...
    # ── 3. Save locally (model/ is gitignored) ───────────────────────────────
